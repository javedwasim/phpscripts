...
//end of function getChildAffaliate($affParentID){
	...
	public function CountAllContacts($affcode = false, $contactType = false) {
		$query = array('Email' => '%');
        $Contact = $this->dsCount("Contact", $query);
        return $Contact;
    }
	...
	public function getallContacts(){
        ...
        while(true)
        {
            $results = $this->dsQuery("Contact", 1000, $page, $query, $returnFields);
	...
	public function updteContactGender($name, $gender) {
	    $returnFields = array('Id','FirstName');
		$query = array('FirstName' => $name);
		$contacts = $this->dsQuery("Contact",10,0,$query,$returnFields);
		foreach($contacts as $contact){
			//echo $name;
			//echo $gender;
			$id = $contact['Id'];
			$condata = array('_Gender00' => $gender);
			$cid = $this->dsUpdate("Contact", $id, $condata);
		}
    }
...
//$contactGlobalObj = new Contacts($name="", $key="");
