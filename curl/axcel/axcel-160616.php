...
if ($app->cfgCon("kq272")) {
    ...
    , 'Phone1', 'StreetAddress1', 'StreetAddress2', 'City', 'State', 'PostalCode', 'Company', 'MiddleName',
        'Title', 'Birthday', '_SourceCode', 'Phone2', '_Gender', '_StreetNumber');
    ...
    $Gender = !empty($contacts[0]['_Gender']) ? $contacts[0]['_Gender'] : '';
    $StreetNumber = !empty($contacts[0]['_StreetNumber']) ? $contacts[0]['_StreetNumber'] : '';
    ...
    echo "<br/>Street No:" . $StreetNumber;
    ...
    if ((!empty($Disability)) && ($Disability > 0)) {
        $data = array(
            ...
            "postcode" => $PostalCode,
            "spostcode" => $PostalCode,
            "city" => $City,
            "scity" => $City,
            "state" => $State,
            "sstate" => $State,
            "streetName" => $StreetAddress1,
            "sstreetName" => $StreetAddress1,
            "sstreetNo" => $StreetNumber,
    ...
    if ((!empty($Homelanguage)) && ($Homelanguage == 1201)) {
        $data['MainLanguageID'] = $Homelanguage;
    }
    ...
    if ((!empty($Schoolyearcompleted))) {
        $data['HighestSchoolLevelYear'] = $Schoolyearcompleted;
    }

    if ((!empty($StreetNumber))) {
        $data['streetNo'] = $StreetNumber;
        $data['sstreetNo'] = $StreetNumber;
    }
...
if (!empty($contact_exist)) {
    ...
    if (!empty($contactId)) {
        ...
        //Create Logtype for course enrollment
        $message = contactEnrol($_REQUEST['contactId'], $CourseInstance, $headers, $app, $contactId, $course_data);
        if (!empty($logType)) {
            contactEnrolLogType($_REQUEST['contactId'], $CourseInstance, $headers, $app, $contactId, $logType);
        }
        ...
        if (!empty($contactId)) {
            ...
            $app->achieveGoal('kq272', 'ErrorNotifications', $_REQUEST['contactId']);
...
function contactEnrolLogType($IScontact_id, $CourseInstance, $headers, $app, $contactId, $data)
{

    $service_url = 'https://admin.axcelerate.com.au/api/course/enrolment';
    $ch = curl_init($service_url);
   ...
   // echo "<pre>";print_r($result);    print_r($result['ERROR']);
    echo "<h4>".$result['MESSAGES']."</h4>";
    if($result['ERROR']==1){
        $condata = array(
