...
class user extends DatabaseObject {
	protected $config=array( // the settings
		...
		'pages'=>array(
			'login'=>'phpscripts/loginscripts/root/login.php', // login page
			'signup'=>'phpscripts/loginscripts/root/signup.php', // registration page
			'manage'=>'manage', // change email page
			'change'=>'phpscripts/loginscripts/root/change.php', // change password page
			'activate'=>'phpscripts/loginscripts/root/activate.php' // activation page
		),
		'site'=>array(
	...
	private function signup(){ // processes the registration form
		...
		$user_count = $database->num_rows($result_set);
		if ($user_count!=0)
		...
		$insert_user = "INSERT INTO `webauth`.`users` (`id`, `name`, `password`, `email`, `temp`) VALUES (NULL, '$name', '$password', '$email', 'a=$activate')";
		$result_set = $database->query($insert_user);
		if($database->insert_id()){
			...
			$message.="\t".'<p><a href="http://'.$_SERVER['SERVER_NAME'].'/'.$this->config['pages']['activate'].'/?activate='.$activate.'">http://'.$_SERVER['SERVER_NAME'].'/'.$this->config['pages']['activate'].'/'.$activate.'</a></p>';
			$this->mail($email,$subject,$message);
			$select_user = "SELECT id FROM users WHERE name='$name' AND password='$password'";
			$result_set = $database->query($sql);
	...
	private function login(){ // processes the login form
		...
		$query_user = "SELECT id, name, email, temp FROM users WHERE name='$name' AND password='$password'";
		$result_set = $database->query($query_user);
		$count_user = $database->num_rows($result_set);
		...
		$this->do_action('login',array($row['id'],$row['name'],$row['email']));
	...
	public function logout(){ // destroys the session to logout
	...
	private function activate(){ // process account and email activation
		global $database;
		$key=$_REQUEST['activate'];
		if(strlen($key)<>32||ereg("[^a-f0-9]",$key)){
			$this->fail('Invalid activation key','/'.$this->config['pages']['login']);
		}
		$keys = array('a='.$key);
		$sql_user = "UPDATE users SET temp='' WHERE temp='$keys[0]'"; 
		$result_set = $database->query($sql_user);
		if(mysql_affected_rows()!=0){
			$this->fail('Your account has been activated','/'.$this->config['pages']['login']);
		}
		
		$sql_select_user = "SELECT id, temp FROM users WHERE temp LIKE '$keys[0]'";
		$result_set = $database->query($sql_select_user);
		$user_count = $database->num_rows($result_set);
	...
	private function change(){ // processes the password recovery form
		...
		if($_POST['login']!=''){
			...
			$sql_select_user = "SELECT id, name, email, temp FROM users WHERE name = '$user'";
			$result_set = $database->query($sql_select_user);
			$user_count = $database->num_rows($result_set);
			...
			$update_user = "UPDATE users SET temp='' WHERE id = '$id'";
			$result_set = $database->query($update_user);
			$user_count = $database->num_rows($result_set);
