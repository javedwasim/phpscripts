...
function parsehtmltable_intomysql($data){
	...
	$drop_table = "DROP TABLE IF EXISTS `affiliatereport`";
	$dbobj->query($drop_table);
	
	//Create table if not exist.
	$sql = "CREATE TABLE IF NOT EXISTS `affiliatereport` ( Id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY, `StatsDate` date NOT NULL, `OfferID` int(11) NOT NULL, `AffiliateID` int(11) NOT NULL, `Impressions` bigint(11) NOT NULL, `Clicks` int(11) NOT NULL, `Conversions` int(11) NOT NULL, `Cost` varchar(100) NOT NULL, `CPC` varchar(100) NOT NULL, `Revenue` varchar(100) NOT NULL, `RPC` varchar(100) NOT NULL, `Profit` varchar(100) NOT NULL ) ENGINE=InnoDB DEFAULT CHARSET=latin1";
	...
	$g->getcells($i);
	  ...
	  for($j = 1; $j<sizeof($report); $j++){
		  ...
		  $insert_query = "INSERT INTO `steptowealth_db`.`affiliatereport` (`Id`, `StatsDate`, `OfferID`, `AffiliateID`, `Impressions`, `Clicks`, `Conversions`, `Cost`, `CPC`, `Revenue`, `RPC`, `Profit`) VALUES (NULL, '$report_data[0]', '$report_data[1]', '$report_data[2]', '$report_data[3]', '$report_data[4]', '$report_data[5]', '$report_data[6]', '$report_data[7]', '$report_data[8]', '$report_data[9]', '$report_data[10]')";
...
function updateAffiliateCode($conid,$affiliateid) {
	
	global $app;
   $condata = array('AffCode' => $affiliateid);
  echo  $cid = $app->dsUpdate("Affiliate", $conid, $condata); exit;
	if (!empty($cid)) {
		return true;
	} else {
		return false;
	...
	/* global $app;
    $affiliatedata = array('AffCode' => "a".$affiliateid);
    $affid = $app->dsUpdate("Affiliate", $conid, $affiliatedata);
	echo $affid; exit;
	if (!empty($affid)) {
		return true;
	} else {
...
function getgenericCommissionPerClick(){ // Get GenericCOmmission for by default entery for Commission Per click 
	$db = new Database();
	$sql = "SELECT genericCommissionPerClick FROM adminsettings WHERE Id=1";
	$result = $db->query($sql);
	$row = $db->fetch_array($result);
	if(!empty($row["genericCommissionPerClick"])){
		return $row["genericCommissionPerClick"];
	}else{
		return false;
	}
}
