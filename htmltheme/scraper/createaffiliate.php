<?php
	...
	if(!empty($contact_info[0]['FirstName'])&&!empty($contact_info[0]['Email'])){
		...
		if(isset($contact_info[0]['Company'])){
			...
			$form3->SetFormValue("data[Affiliate][company]", 'Test Company');
		}
		...
		if(isset($contact_info[0]['Password'])){
			...
			$form3->SetFormValue("data[AffiliateUser][password]", '123456');
		}
		if(isset($contact_info[0]['Password'])){
			$form3->SetFormValue("data[AffiliateUser][password_confirmation]", $contact_info[0]['Password']);
		}else{
			$form3->SetFormValue("data[AffiliateUser][password_confirmation]", '123456');
		}
		...
		if($affiliate_id>0){
			$genericCommissionPerClick = getgenericCommissionPerClick();
			...
			if($count_member==0){
					...
					VALUES ('".$memberid."','".$genericCommissionPerClick."','http://interestingstories.co/','".$affiliate_id."')"; 
					$result = $db->query($sql);	
					$db->close_connection();
			...
			$aff_code_status  = updateAffiliateCode($contactId,$affiliate_id);
			
			echo "<pre>";
			echo "Contatc is authenticated \n";
			print_r($atuhenticate_contact);
			echo "</pre>";
            //---------------------------RUN THE GOAL FOR EMAIL LOGIN DETAIL

            $app->achieveGoal("sv287", "AffiliateLoginDetails", $contactId);

            //-------------------------------------------------------------------
            echo "role updated status ".$role_status."and new password is ".$newpassword;
            $txtfile = 'LogCreateAffaliate.txt';
            $txt = "Dated : " .date('Y-m-d H:i:s').PHP_EOL;
             $txt .= ' Affiliated Id# '.$affiliate_id . ' Created For Contact ID# '.$contactId .PHP_EOL;
		...
		echo "<h4 style='Color:red;'>Following 'FirstName, Email fields require to create affiliate. </h4>";
	}
